...
class WeixinServer {
  ...
  getAccessToken() {
    return (WeixinServer.AccessToken_Url, {
      dataType: 'json',
      method: 'POST',
      contentType: 'json',
      data: {
  ...
  codeToSession(code) {
    return httpclient.request(WeixinServer.Code2Session_Url, {
      dataType: 'json',
      data: {
        appid: this._appid,
        secret: this._secret,
  ...
  getUserEncryptKey({
    ...
    return httpclient.request(WeixinServer.User_Encrypt_Key_Url, {
      dataType: 'json',
      method: 'POST',
  ...
  getH5AccessToken() {
    return httpclient.request(WeixinServer.AccessToken_H5_Url, {
      dataType: 'json',
      method: 'GET',
      data: {
        appid: this._appid,
  ...
  getH5Ticket(access_token) {
    return httpclient.request(WeixinServer.Ticket_Url, {
      dataType: 'json',
      dataAsQueryString: true,
      method: 'POST',
      data: {
  ...
  getH5AccessTokenForEip() {
    return httpProxyForEip.postForm(WeixinServer.AccessToken_H5_Url, {
      appid: this._appid,
      secret: this._secret,
      grant_type: "client_credential"
    }, {
  ...
  getH5TicketForEip(access_token) {
    return httpProxyForEip.postForm(WeixinServer.Ticket_Url, {
      access_token
    }, {
      dataType: 'json',
      dataAsQueryString: true
...
function isAliyun() {
  return (getCloudInfos()[0].provider === 'aliyun')
}
